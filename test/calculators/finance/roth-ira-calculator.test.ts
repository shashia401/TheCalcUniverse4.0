...
describe('roth-ira-calculator', () => {
  ...
  it('returns empty for NaN current age', () => {
    const r = config.calculate({
      currentAge: 'abc',
      retirementAge: '65',
      currentBalance: '15000',
  ...
  it('handles zero starting balance correctly', () => {
    const r = config.calculate({
      currentAge: '30',
      retirementAge: '65',
      currentBalance: '0',
    ...
    // Total contributions should equal 7500 * 35 = 262500
    const totalContribs = parseMoney(getValue(r, 'totalContributions'));
    expect(totalContribs).toBeGreaterThan(200000);
    expect(totalContribs).toBeLessThan(300000);
  });

  it('catches up with missing optional fields gracefully', () => {
    const r = config.calculate({
      currentAge: '30',
      retirementAge: '65',
      annualContribution: '7500',
    ...
    // currentBalance defaults to 0, should still produce results
    expect(r.length).toBeGreaterThan(0);
    expect(getValue(r, 'years')).toContain('35');
  });

  it('growth ratio is a valid percentage', () => {
    const r = config.calculate({
      currentAge: '30',
      retirementAge: '65',
      currentBalance: '15000',
    ...
    const growthRatio = parseNumber(getValue(r, 'growthRatio'));
    expect(growthRatio).toBeGreaterThan(0);
    expect(growthRatio).toBeLessThan(100);
  });

  it('shows higher tax savings with higher retirement tax rate', () => {
    const low = config.calculate({
      currentAge: '30',
      retirementAge: '65',
      currentBalance: '15000',
      annualContribution: '7500',
      annualReturn: '7',
      marginalTaxRate: '22',
      expectedRetirementTaxRate: '10',
    });
    const high = config.calculate({
      currentAge: '30',
      retirementAge: '65',
      currentBalance: '15000',
      annualContribution: '7500',
      annualReturn: '7',
      marginalTaxRate: '22',
      expectedRetirementTaxRate: '32',
    });
    const lowSavings = parseMoney(getValue(low, 'taxSavingsVsTraditional'));
    const highSavings = parseMoney(getValue(high, 'taxSavingsVsTraditional'));
