...
describe('drywall', () => {
  ...
  it('includes screws estimate in results', () => {
    const r = config.calculate({
      length: '10',
      width: '10',
      height: '8',
    ...
    const screws = getValue(r, 'screws');
    expect(screws).toContain('screws');
    // 11 sheets * 32 screws = 352
    near(parseNumber(screws), 352);
  });

  it('includes joint compound estimate', () => {
    const r = config.calculate({
      length: '10',
      width: '10',
      height: '8',
    ...
    const jc = getValue(r, 'jointCompound');
    expect(jc).toContain('gallons');
  });

  it('includes joint tape estimate', () => {
    const r = config.calculate({
      length: '10',
      width: '10',
      height: '8',
    ...
    const tape = getValue(r, 'tape');
    expect(tape).toContain('rolls');
  });
