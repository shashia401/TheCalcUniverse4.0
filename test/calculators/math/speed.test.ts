...
describe('Speed Calculator', () => {
  ...
  it('calcTime: time in seconds', () => {
    const r = config.calculate({ mode: 'calcTime', distance: '100', distanceUnit: 'm', speed: '10', speedUnit: 'ms', timeResultUnit: 'sec' });
    near(parseNumber(getValue(r, 'result')), 10);
    expect(getValue(r, 'result')).toContain('seconds');
  });

  it('calcDistance: kilometers output', () => {
    const r = config.calculate({ mode: 'calcDistance', speed: '60', speedUnit: 'mph', time: '1', timeUnit: 'hr', distanceResultUnit: 'km' });
    near(parseNumber(getValue(r, 'result')), 96.56, 0.1);
    expect(getValue(r, 'result')).toContain('km');
  });

  it('highlighted result is correct color', () => {
    const r = config.calculate({ mode: 'calcSpeed', distance: '60', distanceUnit: 'mi', time: '1', timeUnit: 'hr' });
