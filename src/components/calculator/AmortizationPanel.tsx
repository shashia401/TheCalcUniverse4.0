      ...
      <div className="flex flex-col sm:flex-row items-center gap-6">
        <div className="relative flex-shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-[9px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Monthly</span>
          ...
          {segments.map((seg, i) => (
            <div key={seg.label} className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <span className="text-[10px] text-slate-400 w-8 text-right dark:text-slate-500">{((seg.value / total) * 100).toFixed(0)}%</span>
              </div>
            </div>
          ))}
    ...
    <div className="w-full overflow-x-auto">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 px-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-slate-500 dark:text-slate-400">Balance</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-0.5 rounded-sm flex-shrink-0" style={{ backgroundColor: '#f97316' }} />
          <span className="text-[10px] text-slate-500 dark:text-slate-400">Cumulative Interest</span>
        </div>
      </div>
    </div>
  );
...
function ExtraPaymentPanel({
  ...
  return (
    <div className="border-t border-slate-100 pt-5 space-y-4 dark:border-slate-800">
        ...
        {mode === 'monthly' ? (
          <div className="flex items-center gap-3">
            ...
            <div className="relative flex-1">
              <span className="absolute inset-y-0 left-3 flex items-center text-slate-400 text-sm font-medium pointer-events-none dark:text-slate-500">$</span>
              <input
                type="text"
                inputMode="decimal"
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 col-span-2 sm:col-span-1 dark:border-slate-700 dark:bg-slate-800">
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1 dark:text-slate-400">New Payoff</p>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 dark:bg-slate-900 dark:border-slate-700">
                  <th scope="col" className="text-left px-4 py-2.5 font-bold text-slate-500 uppercase tracking-wider text-[10px] dark:text-slate-400"></th>
                  <th scope="col" className="text-right px-3 py-2.5 font-bold text-slate-500 uppercase tracking-wider text-[10px] dark:text-slate-400">Original</th>
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-3 dark:text-slate-400">Payoff Comparison</p>
      ...
      {savings && savings.timeSavedMonths <= 0 && (
        <p className="text-xs text-slate-400 text-center py-2 dark:text-slate-500">Enter an extra payment amount to see your savings.</p>
      )}
    </div>
  );
}
      ...
      <div className="p-6 space-y-6">
        <div className={`grid gap-6 ${hasExtras ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'}`}>
          {hasExtras && monthlyBreakdown && (
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-3 dark:text-slate-400">Monthly Cost Breakdown</p>
              <CostDonutChart breakdown={monthlyBreakdown} />
            </div>
          )}

          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-slate-500 mb-3 dark:text-slate-400">
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
            {view === 'annual' ? (
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 dark:bg-slate-900 dark:border-slate-700">
                    <th scope="col" className="text-left px-4 py-2.5 font-bold text-slate-500 uppercase tracking-wider text-[10px] dark:text-slate-400">Year</th>
                    <th scope="col" className="text-left px-3 py-2.5 font-bold text-slate-500 uppercase tracking-wider text-[10px] hidden sm:table-cell dark:text-slate-400">Period</th>
                    ...
                    <th scope="col" className="text-right px-4 py-2.5 font-bold text-slate-500 uppercase tracking-wider text-[10px] dark:text-slate-400">Balance</th>
                  </tr>
                </thead>
                <tbody>
                  {displayedYearly.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-slate-400 hidden sm:table-cell dark:text-slate-500">{row.dateRange}</td>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 dark:bg-slate-900 dark:border-slate-700">
                    <th scope="col" className="text-left px-4 py-2.5 font-bold text-slate-500 uppercase tracking-wider text-[10px] dark:text-slate-400">#</th>
                    <th scope="col" className="text-left px-3 py-2.5 font-bold text-slate-500 uppercase tracking-wider text-[10px] dark:text-slate-400">Date</th>
                    <th scope="col" className="text-right px-3 py-2.5 font-bold text-orange-500 uppercase tracking-wider text-[10px]">Interest</th>
                    <th scope="col" className="text-right px-3 py-2.5 font-bold text-blue-500 uppercase tracking-wider text-[10px]">Principal</th>
                    <th scope="col" className="text-right px-4 py-2.5 font-bold text-slate-500 uppercase tracking-wider text-[10px] dark:text-slate-400">Balance</th>
                  </tr>
                ...
                <tbody>
                  {displayedMonthly.map((row, i) => (
                    ...
                    >
                      <td className="px-4 py-2 font-semibold text-slate-500 dark:text-slate-400">{i + 1}</td>
                      <td className="px-3 py-2 text-slate-500 dark:text-slate-400">{MONTH_NAMES[row.month - 1]} {row.year}</td>
