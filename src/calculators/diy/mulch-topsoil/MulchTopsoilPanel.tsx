import { CalculatorResult } from '../../../types/calculator';

interface Props {
  values: Record<string, string>;
  results: CalculatorResult[];
}

function parseNum(s: string): number {
  const m = s.replace(/,/g, '').match(/[\d.]+/);
  return m ? parseFloat(m[0]) : 0;
}

export default function MulchTopsoilPanel({ results }: Props) {
  const cubicYards = results.find(r => r.id === 'cubicYards');
  const cubicFeet = results.find(r => r.id === 'cubicFeet');
  const bags = results.find(r => r.id === 'bags');
  const area = results.find(r => r.id === 'area');

  if (!cubicYards) return null;

  const ydVal = parseNum(cubicYards.value);
  const ftVal = cubicFeet ? parseNum(cubicFeet.value) : 0;
  const bagVal = bags ? parseInt(bags.value) : 0;
  const maxVal = Math.max(ydVal, bagVal * 0.1 || 0);

  const bar = (label: string, val: number, max: number, color: string, unit: string) => {
    const pct = max > 0 ? (val / max) * 100 : 0;
    return (
      <div className="flex items-center gap-3 py-1.5">
        <span className="text-xs font-semibold text-slate-600 w-28 flex-shrink-0">{label}</span>
        <div className="flex-1 h-5 bg-slate-100 rounded overflow-hidden">
          <div className={`h-full rounded ${color} transition-all flex items-center justify-end pr-1.5 ${pct < 10 ? 'min-w-[45px]' : ''}`} style={{ width: `${Math.max(pct, 5)}%` }}>
            <span className="text-[9px] font-bold text-white">{val.toFixed(1)} {unit}</span>
          </div>
        </div>
      </div>
    );
  };

  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-md shadow-slate-200/60 overflow-hidden">
      <div className="flex items-center gap-2 px-6 py-4 border-b border-slate-100 bg-slate-50">
        <svg aria-hidden="true" className="w-4 h-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
        </svg>
        <span className="text-xs font-bold uppercase tracking-widest text-slate-600">Material Volume</span>
      </div>
      <div className="p-5 space-y-2">
        <div className="rounded-xl bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 p-4 text-center">
          <p className="text-xs font-bold uppercase tracking-wider text-green-600">Cubic Yards</p>
          <p className="text-3xl font-bold text-green-700">{cubicYards.value}</p>
        </div>
        {ftVal > 0 && bar('Cubic Feet', ftVal, ftVal, 'bg-blue-500', 'ft')}
        {bagVal > 0 && bar('2 cu ft Bags', bagVal, bagVal, 'bg-amber-500', 'bags')}
        {area && (
          <div className="mt-2 rounded-lg border border-slate-200 px-3 py-2 flex justify-between">
            <span className="text-xs text-slate-600">Area</span>
            <span className="text-sm font-bold text-slate-700">{area.value}</span>
          </div>
        )}
      </div>
    </div>
  );
}
